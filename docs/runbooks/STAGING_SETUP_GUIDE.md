...
A secret-free hardened backend Compose contract exists at `docker/staging/compose.backend.yml` for one digest-pinned migration, API, and worker image plus a private digest-pinned ClamAV service. CI validates its process separation, non-root/read-only application controls, private API/scanner bindings, distinct environment files, `docker/staging/nginx.conf`, and `docker/staging/masarifi-admin.service`; an explicit workflow dispatch publishes the scanned backend image to GHCR. The minimum staging deployment is:
