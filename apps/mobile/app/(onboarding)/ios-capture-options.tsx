...
export default function IosCaptureOptionsRoute() {
  ...
  async function next(result: 'completed' | 'skipped') {
    const progress = await advanceOnboarding(
      ['platform_explanation', 'capture_options'],
      result
    );
  ...
  return (
    ...
    >
      <StyledText>{translate('tracking.ios.limitation')}</StyledText>
      ...
      <ActionButton
        ...
        onPress={() => router.push('/(onboarding)/ios-automation')}
        variant="secondary"
      />
    </OnboardingScaffold>
  );
