...
const sensitiveRoutePattern = /phone|otp|pin|amount|message|email|token|secret|password/i;

function isSafeParameter(value: string) {
  return routeParameterPattern.test(value) && !sensitiveRoutePattern.test(value);
}

export function assistantConversationDestination(conversationId: string): ShellDestination | null {
  return isSafeParameter(conversationId) ? `/assistant/${conversationId}` : null;
}
...
export function supportTicketDestination(ticketId: string): ShellDestination | null {
  return isSafeParameter(ticketId) ? `/support/tickets/${ticketId}` : null;
}

export function sanitizeReturnRoute(route: string | null): ShellDestination | null {
  ...
  const assistantAction = /^\/assistant\/([^/]+)\/actions\/([^/]+)$/.exec(route);
  if (assistantAction) {
    return assistantActionDestination(assistantAction[1], assistantAction[2]);
  }
  const assistantConversation = /^\/assistant\/([^/]+)$/.exec(route);
  if (assistantConversation) return assistantConversationDestination(assistantConversation[1]);
  const supportTicket = /^\/support\/tickets\/([^/]+)$/.exec(route);
  return supportTicket ? supportTicketDestination(supportTicket[1]) : null;
}

export function createReturnContext(destination: '/accounts' | '/assistant', origin: string) {
  const returnTo = sanitizeReturnRoute(origin) ?? '/(tabs)/home';
  return { destination, returnTo };
}

export function tabOrderForDirection(direction: 'ltr' | 'rtl') {
  const tabs = shellDestinations.slice(0, 5);
