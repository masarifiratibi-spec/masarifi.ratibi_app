...
test('monthly periods use inclusive dates and half-open instants', () => {
  ...
  expect(new Date(period.startInstant).toISOString()).toBe('2026-07-31T21:00:00.000Z');
  expect(new Date(period.endExclusiveInstant).toISOString()).toBe('2026-08-31T21:00:00.000Z');
...
test('period instants follow daylight-saving offsets in the captured timezone', () => {
  ...
  expect(new Date(period.startInstant).toISOString()).toBe('2026-03-01T05:00:00.000Z');
  expect(new Date(period.endExclusiveInstant).toISOString()).toBe('2026-04-01T04:00:00.000Z');
